...
protected:
  ...
  static const int mi =
      6; // max_sort_at (actually is equal to mi-1 since C-arrays are started
         // form 0 while the atom indexes are started from 1)
  ...
  // spline coefficients determining functions for pair interactions: the first
  // term in the equation for total energy fi(R_ij)
  ...
  // spline coefficients determining functions for manybody interactions within
  // the centro-symmetric approximation (CSA): the last term in the equation for
  // total energy F(sum_i!=j(rho_ij))
  double **a_sp_emb, **b_sp_emb, **c_sp_emb, **d_sp_emb;
  // spline coefficients determining basis functions f3(R_ij) in the second term
  // describing three-body interactions in the equation for total energy
  double ****a_sp_f3, ****b_sp_f3, ****c_sp_f3, ****d_sp_f3;
  // spline coefficients determining expansion coefficients g3(cos(theta_ijk))
  // for basis functions f3(R_ij) in the third term describing three-body
  // interactions in the equation for total energy
  double a_sp_g3[mi][mg][mf3][mf3], b_sp_g3[mi][mg][mf3][mf3],
      c_sp_g3[mi][mg][mf3][mf3], d_sp_g3[mi][mg][mf3][mf3];
  // spline coefficients determining basis functions f4(R_ij) in the third term
  // describing four-body interactions in the equation for total energy
  double ***a_sp_f4, ***b_sp_f4, ***c_sp_f4, ***d_sp_f4;
  // spline coefficients determining expansion coefficients
  // g4(cos(theta_ijk),cos(theta_jkl),cos(theta_ilj)) for basis functions
  // f4(R_ij) in the third term describing four-body interactions in the
  // equation for total energy
  double **a_sp_g4, **b_sp_g4, **c_sp_g4, **d_sp_g4;
  double **fip_rmin;
  double *z_ion, *c_ZBL, *d_ZBL, **zz_ZBL, **a_ZBL,
      **e0_ZBL; // maybe it is useful to make c_ZBL and d_ZBL three-dimensional
                // arrays
  ...
  bool if_g3_pot; // if two- and three-body interactions are accurately
                  // described by the potential
  bool if_g4_pot; // if two-, three- and four-body interactions are accurately
                  // described by the potential
  bool if_F2_pot; // if two-body interactions are accurately described by the
                  // potential (like within the EAM potential)
  bool *if_gp0_pot;
  int n_sort; // number of sorts of atoms
  int **n_sp_fi, **n_sp_ro, **n_sp_emb, **n_sp_f,
      **n_sp_g; // numbers of spline nodes  for differen
  ...
  /*
   Fortran subroutines translated into the void or double functions (class
   methods) in C++. In the original program md1_temp, all these subrotines use
   the variables stored in the common blocks which are declared in the file
   pot_ls.h. In the LAMMPS implementation, the variables from the Fortran common
   blocks are declared as protected members of PairLS class. The protected
   members of PairLS class can be inherited by any of its child classes.
  */
  ...
  void smooth_zero_22(double *, double, double, double, double, double, double,
                      double, double);
  void SPL(int, double *, double *, int, double, double, double *, double *,
           double *);
  void LA30(int, double *, double *, double *, double *, double *, int *);

  // subroutines for calculating energies and forces
  void e_force_fi_emb(int, int, double *, double **, double **);
  void e_force_g3(int, int, double *, double **, double **);
